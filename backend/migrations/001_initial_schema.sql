...
CREATE TABLE IF NOT EXISTS home_memberships (
    ...
    home_id VARCHAR(128) NOT NULL REFERENCES homes(id) ON DELETE CASCADE,
    user_id VARCHAR(128) NOT NULL REFERENCES users(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS rooms (
    id VARCHAR(128) PRIMARY KEY,
    home_id VARCHAR(128) NOT NULL REFERENCES homes(id) ON DELETE CASCADE,
    floor_id VARCHAR(128) REFERENCES floors(id) ON DELETE SET NULL,
    name VARCHAR(64) NOT NULL,
...
CREATE TABLE IF NOT EXISTS products (
    id VARCHAR(128) PRIMARY KEY,
    family_id VARCHAR(128) NOT NULL REFERENCES product_families(id) ON DELETE RESTRICT,
    display_name VARCHAR(128) NOT NULL,
    description TEXT,
...
CREATE TABLE IF NOT EXISTS devices (
    ...
    product_variant_id VARCHAR(128) NOT NULL REFERENCES product_variants(id) ON DELETE RESTRICT,
...
CREATE TABLE IF NOT EXISTS device_authorizations (
    device_id VARCHAR(128) PRIMARY KEY REFERENCES devices(id) ON DELETE CASCADE,
    home_id VARCHAR(128) NOT NULL REFERENCES homes(id) ON DELETE CASCADE,
    room_id VARCHAR(128) REFERENCES rooms(id) ON DELETE SET NULL,
    custom_name VARCHAR(128) NOT NULL,
    channel_labels JSONB DEFAULT '{}'::jsonb,
    claimed_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    claimed_by_user_id VARCHAR(128) NOT NULL REFERENCES users(id) ON DELETE RESTRICT
);
