...
void main() {
  ...
  group('Capability Engine: Unit & Resolution Tests', () {
    test('CapabilityResolver creates ResolvedDevice with custom labels and states', () {
      const variant = ProductVariantDefinition(
        ...
        channelCount: 3,
        channels: [
          ProductChannelDefinition(channelIndex: 1, defaultLabel: 'Ch 1', capabilities: ['switch', 'relay', 'energy']),
          ProductChannelDefinition(channelIndex: 2, defaultLabel: 'Ch 2', capabilities: ['switch', 'relay', 'fan_speed']),
          ProductChannelDefinition(channelIndex: 3, defaultLabel: 'Ch 3', capabilities: ['switch', 'relay']),
        ],
  ...
  group('Capability Engine: Widget Renderers', () {
    testWidgets('Switch capability renders SwitchControl and responds to taps', (tester) async {
      bool power = false;
    ...
    testWidgets('Fan Speed capability renders FanSpeedControl with 0..5 levels by default', (tester) async {
      int speed = 0;
    ...
    testWidgets('Brightness capability renders BrightnessControl', (tester) async {
      int brightness = 50;
    ...
    testWidgets('CCT capability renders CCTControl with Kelvin display', (tester) async {
      int cct = 4000;
    ...
    testWidgets('Energy capability renders EnergyCard with fixed-point metric conversions', (tester) async {
      ...
      await tester.pumpWidget(
        wrapWidget(
          const EnergyCard(telemetry: telemetry),
        ),
      );
    ...
    testWidgets('DynamicChannelCard renders only supported capabilities and hides missing', (tester) async {
    ...
    testWidgets('DynamicDeviceView renders 3 channels for 3X switch and 1 channel for 1X switch', (tester) async {
      ...
      final device3X = ResolvedDevice(
        ...
        channels: const [
          ResolvedDeviceChannel(channelIndex: 1, name: 'Light 1', capabilities: ['switch']),
          ResolvedDeviceChannel(channelIndex: 2, name: 'Light 2', capabilities: ['switch']),
          ResolvedDeviceChannel(channelIndex: 3, name: 'Light 3', capabilities: ['switch']),
        ],
      );

      await tester.pumpWidget(
        wrapWidget(
          DynamicDeviceView(device: device3X),
        ),
      );
      ...
      final device1X = ResolvedDevice(
        ...
        channels: const [
          ResolvedDeviceChannel(channelIndex: 1, name: 'Main Light', capabilities: ['switch']),
        ],
      );

      await tester.pumpWidget(
        wrapWidget(
          DynamicDeviceView(device: device1X),
        ),
      );

      expect(find.byType(DynamicChannelCard), findsOneWidget);
      expect(find.text('Channels (1)'), findsOneWidget);
      expect(find.byType(EnergyCard), findsNothing); // Hidden when unsupported
    });

    testWidgets('Future Product Extensibility: Custom registered renderer seamlessly renders', (tester) async {
      // Register custom capability renderer for a future product (e.g. Smart Curtain)
      CapabilityRendererRegistry.instance.register('curtain_position', (context, channel, onUpdated) {
        return Container(
      ...
      expect(find.byKey(const ValueKey('custom_curtain_widget')), findsOneWidget);
      expect(find.text('Curtain Position Controller: Balcony Curtain'), findsOneWidget);
    });
  });

  group('Phase 3 Hardening: Metadata-Driven Range & Config Tests', () {
    testWidgets('Fan 3-speed product renders exactly Off, 1, 2, 3 buttons', (tester) async {
      int speed = 0;
      await tester.pumpWidget(
        wrapWidget(
          StatefulBuilder(
      ...
      expect(find.text('4'), findsNothing); // 4 must not exist in 3-speed product
      expect(find.text('5'), findsNothing);
    });

    testWidgets('Fan 5-speed product renders Off, 1, 2, 3, 4, 5 buttons', (tester) async {
      await tester.pumpWidget(
        wrapWidget(
          FanSpeedControl(
            speed: 2,
    ...
    testWidgets('CCT 2200K–6500K range configuration renders correct labels and range', (tester) async {
      await tester.pumpWidget(
        wrapWidget(
          CCTControl(
            kelvin: 3000,
    ...
    testWidgets('CCT 2700K–5700K range configuration renders correct labels', (tester) async {
      await tester.pumpWidget(
        wrapWidget(
          CCTControl(
            kelvin: 4000,
    ...
    testWidgets('EnergyCard with missing/unsupported measurements renders unavailable -- cleanly', (tester) async {
      ...
      await tester.pumpWidget(
        wrapWidget(
          const EnergyCard(telemetry: partialTelemetry),
        ),
      );

      expect(find.text('45.0 W'), findsOneWidget);
      expect(find.text('232.0 V'), findsOneWidget);
      expect(find.text('--'), findsNWidgets(2)); // Missing current and total energy
    });
  });
}
