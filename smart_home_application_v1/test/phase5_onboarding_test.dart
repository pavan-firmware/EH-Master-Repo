...
void main() {
  group('Phase 5B EH-PROV/1 Cryptographic & Onboarding Tests', () {
    ...
    test(
      'Missing commissioningSecret rejects proveIdentity before AUTH',
      () async {
        const service = DefaultOnboardingService();
        ...
        expect(
          proveResult.errorMessage,
          contains('Commissioning secret is required'),
        );
      },
    );
    ...
    test('Malformed or incomplete EH1 payload is rejected', () async {
      ...
      expect(
        res3.errorMessage,
        contains('does not contain required device credentials'),
      );
    });

    test(
      'AUTH computes exact HMAC-SHA256 APP_PROOF using real credential',
      () async {
        const service = DefaultOnboardingService();
        const testSecretHex =
            '0102030405060708090a0b0c0d0e0f101112131415161718191a1b1c1d1e1f20';
        final secretBytes = DefaultOnboardingService.parseSecretKey(
          testSecretHex,
        );
    ...
    test(
      'waitForWifiConnection completes when channel is null or inactive',
      () async {
        const service = DefaultOnboardingService();
