...
class _FakeSseClient extends SseClient {
  ...
  void emitJson(Map<String, dynamic> payload, {String eventType = 'device.state', String id = 'evt_1'}) {
    final envelope = {
      'schemaVersion': 1,
      'eventId': id,
      'type': eventType,
    ...
    _controller.add(SseEvent(id: id, event: eventType, data: jsonEncode(envelope)));
  }
...
void main() {
  ...
  group('Phase 9 — Production Device Lifecycle & State Convergence Tests', () {
    ...
    test('1. Multi-Device Registration & Room Assignment creates distinct rooms and devices', () async {
      ...
      expect(controller.rooms.map((r) => r.name), containsAll(['Living Room', 'Master Bedroom']));
    });

    test('2. Multi-Channel Switch resolves independent channel states', () async {
    ...
    test('3. Physical Switch Override converges state authoritatively via SSE', () async {
      const dev = ConnectedDeviceSummary(
        id: '4444688e-989d-458e-820e-ac62a99ed8e1',
        name: 'EH Smart Switch 3X',
        model: 'eh-smart-switch-3x',
      ...
      expect(controller.getDeviceChannelPower('4444688e-989d-458e-820e-ac62a99ed8e1', 1), isFalse);
    ...
    test('4. App restart rehydration restores complete device and room state without data loss', () async {
      const dev = ConnectedDeviceSummary(
        id: '4444688e-989d-458e-820e-ac62a99ed8e1',
      ...
      expect(rehydratedController.connectionState, HomeConnectionState.connected);
