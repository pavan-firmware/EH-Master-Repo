...
void main() {
  test('preview activity repository exposes six typed events', () async {
    ...
    expect(page.events.map((event) => event.type), contains(ActivityEventType.deviceWarning));
    expect(page.events.map((event) => event.type), contains(ActivityEventType.routineCompleted));
    expect(page.events.map((event) => event.eventTimezone), everyElement('Home timezone'));
    expect(page.events.first.isNavigable, isTrue);
  });

  test('filters and chronological sorting are repository-backed', () async {
    const repository = PreviewActivityRepository();
    final alerts = await repository.getEvents(const ActivityQuery(filter: ActivityFilter.alerts));
    final oldest = await repository.getEvents(const ActivityQuery(sort: ActivitySort.oldest));

    expect(alerts.events, hasLength(2));
    expect(oldest.events.first.timestamp.isBefore(oldest.events.last.timestamp), isTrue);
  });

  testWidgets('activity list opens an event detail page at 360dp', (tester) async {
  ...
  testWidgets('collapsed search expands and filters event content', (tester) async {
    await tester.pumpWidget(const MaterialApp(home: ActivityPage()));
    await tester.pumpAndSettle();
