...
class BleCommissioningChannel {
  ...
  static List<Uint8List> fragmentPayload(Uint8List payload, {int chunkSize = 16}) {
    ...
    for (int i = 0; i < totalFrames; i++) {
      ...
      final end = (start + chunkSize < payload.length) ? start + chunkSize : payload.length;
  ...
  static Uint8List reassembleFrames(List<Uint8List> frames) {
    ...
    for (int i = 0; i < frames.length; i++) {
      ...
      if (frameIndex != i) throw FormatException('Out of order BLE frame index: $frameIndex, expected $i');
