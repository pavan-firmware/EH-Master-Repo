...
class SseClient {
  ...
  Future<void> _connectInternal() async {
    ...
    final token = _apiClient.getAccessToken != null
    ...
    try {
      final uri = Uri.parse(
        '${_apiClient.baseUrl}/api/v1/homes/$_currentHomeId/stream',
      );
      ...
      if (response.statusCode >= 200 && response.statusCode < 300) {
        ...
        // Attempt refresh
        final refreshed = _apiClient.onRefreshToken != null
            ? await _apiClient.onRefreshToken!()
            : false;
        if (refreshed) {
          _connectInternal();
        } else {
