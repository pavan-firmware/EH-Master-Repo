...
def main():
    ...
    add_sym("Connector:Conn_01x03", 35.0, 55.0, "J1", "MAINS_IN", "TerminalBlock_Phoenix:TerminalBlock_Phoenix_MKDS-1,5-3-5.08_1x03_P5.08mm_Horizontal")
    add_label("MAINS_LIVE", 40.08, 52.46)
    add_label("MAINS_NEUTRAL", 40.08, 55.0)
    add_label("EARTH_GND", 40.08, 57.54)

    add_sym("Device:Fuse", 60.0, 52.46, "F1", "2A_250V", "smart_switch_3x:Fuse_TR5", 0)
    add_wire(40.08, 52.46, 56.19, 52.46)
    add_sym("Device:Varistor", 80.0, 55.0, "MOV1", "14D561K", "smart_switch_3x:Varistor_14D", 0)
    ...
    add_sym("Device:R", 100.0, 52.46, "NTC1", "10D-9", "smart_switch_3x:NTC_9D", 90)
    add_wire(80.0, 52.46, 96.19, 52.46)
    add_sym("Device:C", 115.0, 55.0, "CX1", "0.1uF_275V_X2", "smart_switch_3x:Capacitor_Box_P10mm", 0)
    ...
    add_sym("Power_Supply:HLK-PM01", 185.0, 55.0, "U2", "HLK-PM01_5V", "smart_switch_3x:HLK-PM01")
    ...
    add_sym("Device:C_Polarized", 215.0, 55.0, "C1", "470uF_16V", "Capacitor_THT:CP_Radial_D6.3mm_P2.50mm", 0)
    ...
    add_sym("Regulator_Linear:AP2112K-3.3", 240.0, 55.0, "U3", "AP2112K-3.3", "Package_TO_SOT_SMD:SOT-23-5")
    add_wire(215.0, 49.92, 229.84, 52.46)
    add_wire(229.84, 52.46, 229.84, 57.54) # tie VIN and EN
    add_pwr("power:GND", 240.0, 65.0, "GND")
    add_wire(240.0, 62.62, 240.0, 65.0)

    add_sym("Device:C_Polarized", 260.0, 55.0, "C2", "100uF_10V", "Capacitor_SMD:CP_Elec_6.3x5.8", 0)
    ...
    # Block 3: MCU Core
    add_sym("RF_Module:ESP32-C6-WROOM-1", 65.0, 135.0, "U1", "ESP32-C6-WROOM-1-N4", "smart_switch_3x:ESP32-C6-WROOM-1")
    add_pwr("power:+3V3", 49.76, 119.76, "+3V3")
    add_pwr("power:GND", 65.0, 165.0, "GND")
    add_wire(65.0, 160.4, 65.0, 165.0)

    # MCU Reset Circuit
    add_sym("Device:R", 38.0, 124.84, "R_EN2", "10k", "Resistor_SMD:R_0603_1608Metric", 0)
    add_pwr("power:+3V3", 38.0, 118.0, "+3V3")
    add_wire(38.0, 121.03, 49.76, 124.84)
    add_sym("Device:C", 38.0, 135.0, "C_EN2", "1uF", "Capacitor_SMD:C_0603_1608Metric", 0)
    ...
    add_sym("Device:R", 100.0, 150.24, "R_LED2", "1k", "Resistor_SMD:R_0603_1608Metric", 90)
    add_label("STATUS_LED", 93.0, 150.24, 180)
    add_wire(93.0, 150.24, 96.19, 150.24)
    add_sym("Device:LED", 112.0, 150.24, "LED1", "BLUE_0603", "LED_SMD:LED_0603_1608Metric", 0)
    ...
    for i, y_pos in enumerate(y_relays):
        ch = i + 1
        # MOSFET
        add_sym("Device:Q_NMOS_GSD", 185.0, y_pos, f"Q{ch}", "AO3400A", "Package_TO_SOT_SMD:SOT-23")
        add_label(f"RELAY_DRV_{ch}", 170.0, y_pos, 180)
        add_sym("Device:R", 175.0, y_pos, f"RG{ch}", "1k", "Resistor_SMD:R_0603_1608Metric", 90)
        add_wire(170.0, y_pos, 171.19, y_pos)
        add_wire(178.81, y_pos, 179.92, y_pos)
        add_sym("Device:R", 179.92, y_pos+6.0, f"RPD{ch}", "10k", "Resistor_SMD:R_0603_1608Metric", 0)
        ...
        # Relay & Flyback Diode
        add_sym("Relay:HF32F-G", 215.0, y_pos, f"K{ch}", "HF32F-G_10A", "smart_switch_3x:Relay_HF32F-G")
        ...
        add_sym("Device:D_Schottky", 200.0, y_pos, f"D{ch}", "SS14", "Diode_SMD:D_SOD-123", 90)
        add_wire(207.38, y_pos-2.54, 200.0, y_pos-3.81)
        add_wire(207.38, y_pos+2.54, 200.0, y_pos+3.81)

        # Snubber
        add_sym("Device:R", 235.0, y_pos-2.54, f"RS{ch}", "100_1W", "Resistor_SMD:R_2512_6332Metric", 90)
        add_sym("Device:C", 248.0, y_pos-2.54, f"CS{ch}", "100nF_630V", "smart_switch_3x:Capacitor_Box_P10mm", 90)
    ...
    add_sym("Sensor_Energy:BL0942", 65.0, 215.0, "U4", "BL0942_SOP14", "Package_SO:SOIC-14_3.9x8.7mm_P1.27mm")
    ...
    add_sym("Device:R", 30.0, 212.46, "R_SHUNT2", "1mOhm_2W_1%", "Resistor_SMD:R_2512_6332Metric", 0)
    ...
    add_sym("Device:R", 38.0, 222.62, "RHV1", "960k_HV", "Resistor_SMD:R_1206_3216Metric", 90)
    ...
    for i, y_pos in enumerate(y_sw):
        ch = i + 1
        add_sym("Connector:Conn_01x02", 170.0, y_pos, f"J_SW{ch}", f"WALL_SW_{ch}", "smart_switch_3x:TerminalBlock_2P_P3.5mm")
        add_label(f"SW_IN_{ch}", 225.0, y_pos)
        add_sym("Device:R", 185.0, y_pos, f"RSW{ch}", "1k", "Resistor_SMD:R_0603_1608Metric", 90)
        add_wire(175.08, y_pos, 181.19, y_pos)
        add_wire(188.81, y_pos, 225.0, y_pos)
        
        # Pull-Up
        add_sym("Device:R", 195.0, y_pos-6.0, f"RPU{ch}", "10k", "Resistor_SMD:R_0603_1608Metric", 0)
        add_wire(195.0, y_pos, 195.0, y_pos-2.19)
        add_pwr("power:+3V3", 195.0, y_pos-12.0, "+3V3")
        add_wire(195.0, y_pos-9.81, 195.0, y_pos-12.0)

        # Cap & TVS to GND
        add_sym("Device:C", 205.0, y_pos+6.0, f"CSW{ch}", "100nF", "Capacitor_SMD:C_0603_1608Metric", 0)
        add_wire(205.0, y_pos, 205.0, y_pos+2.19)
        add_pwr("power:GND", 205.0, y_pos+12.0, "GND")
        add_wire(205.0, y_pos+9.81, 205.0, y_pos+12.0)

        add_sym("Device:D_Schottky", 215.0, y_pos+6.0, f"TVS{ch}", "ESD5Z3.3", "Diode_SMD:D_SOD-523", 270)
    ...
    with open(r'c:\Users\pavan\Downloads\Flutter\SMART_HOME_V1\hardware\kicad\smart_switch_3x\smart_switch_3x.kicad_sch', 'w', encoding='utf-8') as f:
        f.write(content)
    print("Successfully generated smart_switch_3x.kicad_sch with footprints assigned!")
